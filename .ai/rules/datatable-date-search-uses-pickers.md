...
3. Render native `type="date"` pickers (or an equivalent date-range control) in the toolbar.
4. Server-side, interpret from/to as **Asia/Manila** calendar days and convert to UTC bounds for the query.

## Reference

See Officer Payments (`recorded` range on `created_at`) and `DataTable`’s `dateRanges` prop.
